 ...
 * the merged single package carries the pentest host plugin, so this
 * companion checks the exploration discipline — every pentest write honors
...
import type { PentestEdgeKind } from './dsh-pentest/src/spec.ts'
...
const SOURCE_TABLE_OF_EDGE: Record<PentestEdgeKind, 'goals' | 'intents' | 'facts' | 'assets'> = {
  spawns: 'goals',
  yields: 'intents',
  derived_from: 'facts',
  proves: 'intents',
...
const TARGET_TABLE_OF_EDGE: Record<PentestEdgeKind, 'intents' | 'facts' | 'findings' | 'assets'> = {
  spawns: 'intents',
  yields: 'facts',
...
const install: InvariantInstaller = Object.assign((ctx: Context, fail: InvariantFailure) => {
  ctx.on('domain/changed', (change: DomainChanged) => {
    ...
    if (change.table === 'edges') {
      const edge = record as unknown as { readonly kind: PentestEdgeKind; readonly sourceId: string; readonly targetId: string }
